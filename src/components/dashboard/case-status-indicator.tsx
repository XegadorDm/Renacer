import { CaseStatus } from "@/lib/case-schema";
import { cn } from "@/lib/utils";

const statusColors: Record<CaseStatus, string> = {
  "Sin novedad": "bg-red-600",
  "Respuesta Gobierno en curso": "bg-amber-500",
  "Proceso finalizado con exito": "bg-emerald-600",
};

export function CaseStatusIndicator({ status }: { status: CaseStatus }) {
  const colorClass = status in statusColors ? statusColors[status] : "bg-slate-400";

  return (
    <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-background border shadow-sm w-fit">
      <span className={cn("h-2.5 w-2.5 rounded-full shadow-inner", colorClass)} />
      <span className="text-[10px] font-black uppercase tracking-widest text-foreground/80 leading-none">
        {status}
      </span>
    </div>
  );
}